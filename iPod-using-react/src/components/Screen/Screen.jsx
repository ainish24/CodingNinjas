...
const Screen = ({showMenu, Options, selectedOption, changeSelected, highlightedOption, changeHighlighted}) => {
  const imgUrl =
        ...
        {showMenu && (<Menu>
          ...
          <MenuList>
            {Options.map((option, index) => {
              return (
                <StyledLi
                  className={option.text == highlightedOption ? "selected" : ""}
                  key={index}
                  onClick={() => changeSelected(option.text)}
                >
                  {option.text}
