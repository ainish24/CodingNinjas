...
// Create your function here to handle post creation and adding Post to the postsData.
  ...
  let newImage=''
  const formImageInput = document.getElementById("imageInput")
  formImageInput.addEventListener("change", function(event){
    const imgFile= event.target.files[0];
    if(imgFile){
      newImage= URL.createObjectURL(imgFile)
    }
  })
// // Add Event listeners to listen to the submit event of the form.
  formElement.addEventListener("submit",()=>{
    event.preventDefault()
    const formInput = document.getElementById("postInput")
  
    console.log(newImage)
    const newId = postsData.length + 1
    const newAuthor = "You"
    const newContent = formInput.value
    const newLikes = 0
    const newComments = []
    const newPost={
      id: newId,
